...
"""
TC.py  (KST 버전 — TCA_GES.py 에서 파생)
================================
SM2RAIN + ERA5 + GPM TCA 기반 강수량 병합 (전부 KST 일경계 입력)
  - 데이터 1 (active)  : SM2RAIN 강수량   ← SM2RAIN_GA_KST.nc
  - 데이터 2 (model)   : ERA5-Land 강수량 ← era5_land{yr}_KST.nc (70×70 → 49×49 in-memory regrid)
  - 데이터 3 (passive) : GPM 강수량       ← GPM_{yr}_KST.nc
  - 검증 기준          : IDW_ASOS (지상관측 = 원래 KST)
ERA5 KST native(70×70) 파일은 건드리지 않고, 이 코드 안에서만 SM2RAIN 격자로 보간한다.

* TCA 피팅(오차분산·가중치·ERA5/GPM 재스케일 계수)은 2021년 표본만으로 추정하고,
...
# 지상 IDW(AWS·ASOS)는 단일 파일 da_IDWs.nc 사용 (V-0 의 IDW_BOTH_PATH 참조)
...
GPM_KST_DIR  = os.path.join(KST_DIR, '')                               # GPM_{yr}_KST.nc
ERA5_KST_DIR = os.path.join(KST_DIR, 'data/era5_KST')                              # era5_land{yr}_KST.nc

OUT_MERGE_PATH   = os.path.join(BASE_PATH, 'TCA/TC_KST_T.nc')
OUT_WEIGHTS_PATH = os.path.join(BASE_PATH, 'TCA/TC_weights_KST_T.nc')
...
#   ERA5 KST native(70×70) → SM2RAIN 격자(49×49) in-memory regrid
# ============================================================
...
def _regrid_bilinear(cube, src_lat, src_lon, tgt_lat, tgt_lon, min_cov=0.5):
    """cube (nlat,nlon,T) → (TL,TO,T). NaN-aware: 바다(NaN)는 가중 재정규화로 제외.
    0*NaN=NaN 오염을 막기 위해 NaN→0 후, 유효 가중합(den)으로 나눠 정규화한다.
    min_cov 미만(보간 가중의 절반 미만이 유효)인 타깃 픽셀은 NaN 처리."""
    ...
    num = _apply_bilinear(cube0, Mlat, Mlon)           # 유효값 가중합
    den = _apply_bilinear(valid, Mlat, Mlon)           # 유효 가중합(커버리지)
    out = np.where(den > min_cov, num / np.where(den == 0, np.nan, den), np.nan)
    return out


def load_era5_kst_regrid(years, era5_dir, tgt_lat, tgt_lon):
    """연도별 era5_land{yr}_KST.nc(이미 daily mm/day, 70×70) 로드·concat → SM2RAIN 격자로 보간."""
    arrs, times = [], []
    ...
    cube = np.concatenate(arrs, axis=2)                 # (70,70,Tall) native
    t = times[0]
    for x in times[1:]:
        t = t.append(x)
    # SM2RAIN 격자(오름차순)로 보간
    tl = np.sort(tgt_lat); to = np.sort(tgt_lon)
    rg = _regrid_bilinear(cube, src_lat, src_lon, tl, to)   # (49,49,Tall)
    rg[rg < 0] = np.nan
    return rg, tl, to, t
...
def load_era5_daily(path):
    """ERA5 hourly accum(m) → daily total(mm/day)
    stepType=accum: 예보 시작(00:00 UTC)부터 누적
    → 다음날 00:00 = 전날 24시간 완전 누적 = 일합계
    시간 인덱스를 하루 앞당겨 해당 날짜에 배정, m → mm 변환
    """
    ds  = xr.open_dataset(path)
    var = next(v for v in ds.data_vars
               if v.lower() not in {'lat', 'lon', 'latitude', 'longitude', 'time'})
    ...
    # ── 00:00 타임스텝 선택 = 전날 24시간 누적 일합계 ───────────
    # 예보가 00:00 UTC 시작 → 다음날 00:00 = 24h 완전 누적
    # 예: 2025-01-10 00:00 값 → 2025-01-09 일합계
    da_daily = da.sel(time=da['time'].dt.hour == 0)

    # m → mm 변환
    arr = da_daily.values.astype(np.float64) * 1000.0

    # 시각 → 날짜 인덱스: 하루 앞당겨서 해당 날짜로 배정
    time_daily = pd.DatetimeIndex(da_daily['time'].values).normalize() - pd.Timedelta(days=1)

    dims    = [d.lower() for d in da_daily.dims]
    lat_ax  = next((i for i, d in enumerate(dims) if 'lat' in d or d == 'y'), None)
    lon_ax  = next((i for i, d in enumerate(dims) if 'lon' in d or d == 'x'), None)
    time_ax = next((i for i, d in enumerate(dims) if 'time' in d), None)
    ...
    arr = np.transpose(arr, (lat_ax, lon_ax, time_ax))
    ...
    arr[arr < 0] = np.nan
    return arr, lat_1d, lon_1d, time_daily
...
gpm_arrs, gpm_times = [], []
for yr in YEARS:
    arr, lat_gpm, lon_gpm, t = load_precip(os.path.join(GPM_KST_DIR, f'GPM_{yr}_KST.nc'))
    gpm_arrs.append(arr)
    gpm_times.append(t)
    print(f"  GPM_{yr}_KST: {arr.shape}  {t[0].date()} ~ {t[-1].date()}")

arr_gpm  = np.concatenate(gpm_arrs, axis=2)
time_gpm = gpm_times[0]
for t in gpm_times[1:]:
    time_gpm = time_gpm.append(t)
print(f"  → GPM 전체: {arr_gpm.shape}  {time_gpm[0].date()} ~ {time_gpm[-1].date()}")

# GPM 격자(0.125°, 49×49)가 SM2RAIN 격자(ASCAT 58×46)와 다르면 in-memory 재격자
if arr_gpm.shape[:2] != arr_sm.shape[:2] or not (
        np.allclose(lat_sm, lat_gpm, atol=0.01) and np.allclose(lon_sm, lon_gpm, atol=0.01)):
    print(f"  GPM 재격자: {arr_gpm.shape[:2]} → {arr_sm.shape[:2]} (SM2RAIN 격자)")
    arr_gpm = _regrid_bilinear(arr_gpm, lat_gpm, lon_gpm, np.sort(lat_sm), np.sort(lon_sm))
    arr_gpm[arr_gpm < 0] = np.nan
    lat_gpm, lon_gpm = np.sort(lat_sm), np.sort(lon_sm)
print()

# ── ERA5 (연도별 KST + SM2RAIN 격자로 in-memory regrid) ───────
print("[ ERA5 (KST, 70×70 → 49×49 regrid) ]")
...
assert arr_sm.shape[:2] == arr_gpm.shape[:2] == arr_era5.shape[:2], \
    f"격자 크기 불일치: SM={arr_sm.shape[:2]}, GPM={arr_gpm.shape[:2]}, ERA5={arr_era5.shape[:2]}"
assert np.allclose(lat_sm, lat_gpm,  atol=0.01), "lat 불일치 (SM2RAIN vs GPM)"
assert np.allclose(lon_sm, lon_gpm,  atol=0.01), "lon 불일치 (SM2RAIN vs GPM)"
...
common_dates = time_sm.intersection(time_era5).intersection(time_gpm)
...
print(f"  GPM     : {time_gpm[0].date()} ~ {time_gpm[-1].date()}  ({len(time_gpm)}일)")
...
X_gpm  = select_dates(arr_gpm,  time_gpm,  common_dates)
T = len(common_dates)
print(f"  공통 기간 배열: SM={X_sm.shape}, ERA5={X_era5.shape}, GPM={X_gpm.shape}")
...
err2_gpm  = np.full(shape, np.nan)
n_valid_map = np.zeros(shape, dtype=int)

for i in tqdm(range(n_lat), desc="  위도줄"):
    for j in range(n_lon):
        ex, ey, ez, nv    = dong_tca_pixel(A_sm_fit[i,j,:], A_era5_fit[i,j,:], A_gpm_fit[i,j,:], MIN_TRIPLET)
        err2_sm[i,j]   = ex
        err2_era5[i,j] = ey
        err2_gpm[i,j]  = ez
        n_valid_map[i,j] = nv
...
# 음(–)/0 오차분산 대치(구멍 방지): NaN 으로 버리면 그 육지 픽셀이 병합에서 빠져
#   지도에 구멍이 생김. 0 근처 바닥처리는 그 자료 가중치를 1로 쏠리게 하므로,
#   '그 자료 양수 분산 중앙값'으로 대치. (바다/자료없음 픽셀은 원래 NaN → 대치 안 함)
for _e in (err2_sm, err2_era5, err2_gpm):
...
print(f"  σ²_GPM     : mean={np.nanmean(err2_gpm):.4f}  n_valid={np.sum(np.isfinite(err2_gpm))}")
...
denom = err2_sm*err2_era5 + err2_era5*err2_gpm + err2_sm*err2_gpm

w_sm   = err2_era5 * err2_gpm / denom
w_era5 = err2_sm   * err2_gpm / denom
w_gpm  = err2_sm   * err2_era5 / denom

valid_w = np.isfinite(w_sm) & np.isfinite(w_era5) & np.isfinite(w_gpm)
...
print(f"  w_GPM     mean={np.nanmean(w_gpm):.3f}")
print(f"  w_sum check: mean={np.nanmean(w_sm + w_era5 + w_gpm):.6f} (≈1.0)")
...
scale_gpm  = np.full(shape, np.nan)

for i in range(n_lat):
    for j in range(n_lon):
        # 재스케일 계수도 2021 표본으로만 추정
        s = A_sm_fit[i, j, :];  a = A_era5_fit[i, j, :];  g = A_gpm_fit[i, j, :]
...
ds_merge = xr.Dataset(
    data_vars={
        "precipitation": xr.DataArray(
            ...
            dims=("time", "lat", "lon"),
            attrs={"long_name": "TCA-merged precipitation (SM2RAIN + ERA5 + GPM)",
                   "units": "mm/day",
    ...
    attrs={"title":  "TCA Merged Precipitation (KST)",
           "inputs": "SM2RAIN_GA_KST + ERA5_KST(regrid) + GPM_KST",
...
ds_weights = xr.Dataset(
    data_vars={
        ...
        "err2_GPM":     xr.DataArray(err2_gpm,   dims=("lat","lon"),
            attrs={"long_name": "TCA error variance (GPM)",     "units": "mm²/day²"}),
        ...
        "w_GPM":        xr.DataArray(w_gpm,       dims=("lat","lon"),
            attrs={"long_name": "TCA weight GPM"}),
        "scale_ERA5":   xr.DataArray(scale_era5,  dims=("lat","lon"),
            attrs={"long_name": "Scaling parameter ax/ay (ERA5→SM2RAIN ref)"}),
        "scale_GPM":    xr.DataArray(scale_gpm,   dims=("lat","lon"),
            attrs={"long_name": "Scaling parameter ax/az (GPM→SM2RAIN ref)"}),
...
# 단일 파일 da_IDWs.nc (변수 'AWS (IDW)'/'ASOS (IDW)', 좌표 y/x 2D, dims time,y,x)
# 파일은 그대로 두고 배열만 (lat, lon, time) 오름차순으로 정리해 사용.
IDW_BOTH_PATH = "/Users/kim/DAS/projects/KIHS/IDW/both/da_IDWs.nc"

def load_idw_both(varname_sub):
    """da_IDWs.nc 에서 varname_sub('aws'/'asos') 포함 변수를 (lat,lon,time)으로 반환."""
...
AWS_val_full, lat_ref, lon_ref, time_aws_val = load_idw_both('asos')
print(f"  IDW_ASOS(da_IDWs): {AWS_val_full.shape}  {time_aws_val[0].date()}~{time_aws_val[-1].date()}")
assert AWS_val_full.shape[:2] == arr_sm.shape[:2], \
    f"검증자료 격자 불일치: ASOS={AWS_val_full.shape[:2]} vs SM2RAIN={arr_sm.shape[:2]}"

aws_mask = time_aws_val.isin(common_dates)
gpm_mask = time_gpm.isin(common_dates)
sm_mask  = time_sm.isin(common_dates)

REF_val  = AWS_val_full[:, :, aws_mask]   # 검증 기준 (IDW_ASOS)
GPM_val  = arr_gpm[:, :, gpm_mask]
SM_val   = arr_sm[:, :, sm_mask]
ERA5_val = X_era5                         

# ── 남한(South Korea) 마스크: 검증 계산·그림은 남한 육지 픽셀만 ──
#    Korea.shp(시도 경계, EPSG:5179) → 위경도 변환 후 픽셀 중심 포함 판정.
#    격자(58×46, ~38.7°N)가 북한 남부까지 덮으므로, 관측(IDW)이 외삽되는
#    북한 지역은 검증·그림에서 제외한다. (병합 산출물 nc 는 이미 저장됨)
...
for _a in (REF_val, GPM_val, SM_val, ERA5_val, P_merged):     # 3D (lat,lon,T)
    _a[~SK_MASK, :] = np.nan
for _a in (err2_sm, err2_era5, err2_gpm, w_sm, w_era5, w_gpm):  # 2D 진단(F-1·F-2)
    _a[~SK_MASK] = np.nan

print(f"  P_merged : {P_merged.shape}")
print(f"  REF_val  : {REF_val.shape}  ← IDW_ASOS (독립 검증 기준)")
print(f"  GPM_val  : {GPM_val.shape}")
...
R_gpm     = pixelwise_corr(GPM_val,   REF_val)
...
print(f"  R_GPM     : mean={np.nanmean(R_gpm):.3f}  median={np.nanmedian(R_gpm):.3f}")
...
STYLE = {
    ...
    'GPM':     dict(color='#FF9800', lw=0,   ls='',   marker='^',  ms=5, label='GPM',               alpha=0.65, zorder=2),
...
TS_STYLE = {
    ...
    'GPM':     dict(color='#FF9800', lw=0.9, ls='-.', label='GPM',               alpha=0.85, zorder=2),
...
ev_cfg = [
    (err2_sm,   'SM2RAIN', 0, 80),   # (data, label, vmin, vmax)
    (err2_era5, 'ERA5',    0, 80),
    (err2_gpm,  'GPM',     0, 80),
]
...
plt.suptitle('F-1.  TCA Error Variance 공간분포  (SM2RAIN + ERA5 + GPM)',
             fontsize=13, fontweight='bold', y=1.02)
fig.savefig(os.path.join(FIG_DIR, 'F1_error_variance_KST.png'), dpi=150, bbox_inches='tight')
plt.show()
...
w_sum = np.nansum(np.stack([w_sm, w_era5, w_gpm], axis=0), axis=0)
plt.suptitle(f'F-2.  TCA Weights 공간분포  (w_sum mean={np.nanmean(w_sum[w_sum>0]):.3f})',
             fontsize=13, fontweight='bold', y=1.02)
fig.savefig(os.path.join(FIG_DIR, 'F2_weights_KST.png'), dpi=150, bbox_inches='tight')
plt.show()
...
for stat_key, stat_name, cmap, cb_lbl in stat_cfgs:  # ← FIX: fname 제거
    ...
    for col_idx, (ds_label, _) in enumerate(COL_DATA):
        ...
        ax.text(0.02, 0.03, 'vs IDW_ASOS',  # ← FIX: IDW_ASOS 
    ...
    fig.suptitle(f'F-3.  {stat_name}  —  GPM / SM2RAIN / ERA5 / Merged (TCA)  vs  IDW_ASOS',
                 fontsize=13, fontweight='bold')
    fig.savefig(os.path.join(FIG_DIR, f'F3_{stat_key}_vs_IDW_ASOS_KST.png'),
...
legend_ts = [
    ...
    for k in ['IDW_ASOS', 'SM2RAIN', 'GPM', 'ERA5', 'Merged']
]
...
for basin_name, (lat_min, lat_max, lon_min, lon_max) in BASINS.items():
    ...
    ts_gpm  = _bmean(GPM_val,  bmask)
    ts_era5 = _bmean(ERA5_val, bmask)   # ← IDW_ASOS → ERA5
    ...
    ax_ts.plot(date_arr, ts_gpm,  **TS_STYLE['GPM'])
    ...
    lim_b = 0.0
    for key, ts_pred in [('SM2RAIN', ts_sm), ('GPM', ts_gpm),
    ...
    ax_b.set_title(f'(b) {basin_name}\nvs IDW_AWS', fontsize=10, fontweight='bold')
    ...
    for key, ts_pred, fc in [
        ...
        ('GPM',     ts_gpm,  '#FF9800'),
        ('ERA5',    ts_era5, '#27ae60'),   # ← IDW_ASOS → ERA5
    ...
    fig.savefig(os.path.join(FIG_DIR, f'F4_{safe_name}_KST.png'), dpi=150, bbox_inches='tight')
    plt.show()
